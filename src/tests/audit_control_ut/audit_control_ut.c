...
TEST_FUNCTION(AuditControl_AddRule_64BitHost_ExpectSuccess)
{
    ...
    STRICT_EXPECTED_CALL(sysconf(_SC_LONG_BIT)).SetReturn(64);
    STRICT_EXPECTED_CALL(audit_rule_fieldpair_data(IGNORED_PTR_ARG, "arch=b64", AUDIT_FILTER_EXIT & AUDIT_FILTER_MASK)).SetReturn(0);
    ...
    AuditControlResultValues result = AuditControl_AddRule(&audit, testSyscalls, 1, ARCHITECTURE_64, NULL);
...
TEST_FUNCTION(AuditControl_AddRule_Non64BitHost_ExpectSuccess)
{
    ...
    STRICT_EXPECTED_CALL(audit_rule_fieldpair_data(IGNORED_PTR_ARG, "arch=b32", AUDIT_FILTER_EXIT & AUDIT_FILTER_MASK)).SetReturn(0);
    STRICT_EXPECTED_CALL(audit_rule_syscallbyname_data(IGNORED_PTR_ARG, "msg")).SetReturn(0);
    STRICT_EXPECTED_CALL(audit_add_rule_data(audit.audit, IGNORED_PTR_ARG, AUDIT_FILTER_EXIT, AUDIT_ALWAYS)).SetReturn(1);

    const char* testSyscalls[] = {"msg"};
    AuditControlResultValues result = AuditControl_AddRule(&audit, testSyscalls, 1, ARCHITECTURE_32, NULL);
...
TEST_FUNCTION(AuditControl_AddRule_ExtraFilter_ExpectSuccess)
{
    ...
    const char* testSyscalls[] = {"msg"};
    AuditControlResultValues result = AuditControl_AddRule(&audit, testSyscalls, 1, ARCHITECTURE_32, "a=b");
...
TEST_FUNCTION(AuditControl_AddRule_Expectfailure)
{
    ...
    STRICT_EXPECTED_CALL(sysconf(_SC_LONG_BIT)).SetReturn(64);
    STRICT_EXPECTED_CALL(audit_rule_fieldpair_data(IGNORED_PTR_ARG, "arch=b64", AUDIT_FILTER_EXIT & AUDIT_FILTER_MASK)).SetFailReturn(1);
    ...
    for (int i = 0; i < umock_c_negative_tests_call_count(); i++) {
        ...
        const char* testSyscalls[] = {"msg"};
        AuditControlResultValues result = AuditControl_AddRule(&audit, testSyscalls, 1, ARCHITECTURE_64, "donald=duck");
        ASSERT_ARE_EQUAL(int, AUDIT_CONTROL_EXCEPTION, result);
    }
